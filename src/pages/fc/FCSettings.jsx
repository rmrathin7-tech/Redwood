...
const BLOCK_DEFAULTS = {
  table: {
    cols: 2, numRows: 1, baseRowCount: 1, editableHeaders: true, hasTotalsRow: false, showSno: false,
    showColumnTotals: false, allowNA: false, allowAddRows: true, allowInsertRows: false, allowInsertCols: false,
    allowDeleteCols: true, allowRepeatTable: false, allowInstanceNames: false, protectTotalsRow: true,
    colHeaders: ['Column 1', 'Column 2'], rows: [], enableTableSubheading: false, tableSubheadingRichText: '',
  },
  chart: {
    title: 'Revenue by Quarter', chartType: 'bar', xAxisLabel: 'Quarter', series: ['Actual', 'Projected'],
    colors: ['3b82f6', '10b981'], showLegend: true, allowAddRows: true, baseRowCount: 1,
    pieSeriesIndex: 0, pieColors: ['3b82f6', '10b981', 'f59e0b', '8b5cf6', 'ec4899'],
  },
  'conditional-switch': {
  ...
  'repeating-block-set': { addLabel: 'Add Set', blocks: [] },
  'repeating-group': {
    template: [ { id: 'title', label: 'Name', type: 'text' }, { id: 'desc', label: 'Description', type: 'textarea' } ],
    addLabel: 'Add Entry', allowImage: false,
  },
  ...
  image: { multiple: false, allowCaption: true, imageWidth: '100%', imageHeight: '180px', objectFit: 'cover' },
  file:       { multiple: false },
};
...
export default function FCSettings() {
  ...
  // FIX: Using 'fc' instead of 'im' for the route parameter
  const fcId = searchParams.get('fc'); 
  ...
  useEffect(() => {
    const loadSchema = async () => {
      // FIX: Changed 'im-schema' to 'fc-schema'. This gives FC its own independent state!
      const snap = await getDoc(doc(db, 'config', 'fc-schema'));
      if (snap.exists()) {
  ...
  const handleExportJSON = () => {
    ...
    dlAnchorElem.setAttribute("download", `fc_schema_${projectId || 'export'}.json`);
    dlAnchorElem.click();
  };
  ...
  const handleAddBlock = (typeId) => {
    ...
    if (typeId === 'table') extraConfig.rows = seedTableRows();
  ...
  const duplicateBlock = (block) => {
    ...
    copy.id = uid; copy.dataPath = `block_${uid}`; copy.label = `${copy.label} (copy)`; copy.order = block.order + 0.5;
    setSections(sections.map(sec => {
      if (sec.id !== activeSectionId) return sec;
  ...
  const handleSave = async () => {
    ...
    setSaveMsg('');
    try {
      // FIX: Changed from 'im-schema' to 'fc-schema'
  ...
  const T = {
    ...
    primary:       isDark ? '#f59e0b'                : '#d97706', // FIX: Changed primary color to Amber for FC
    primaryLight:  isDark ? 'rgba(245,158,11,0.08)'  : 'rgba(217,119,6,0.08)',
    primaryBorder: isDark ? 'rgba(245,158,11,0.25)'  : 'rgba(217,119,6,0.3)',
    danger:        '#ef4444',
  ...
  const renderTableConfigUI = (baseConfig, onChangeConfig, pathPrefix) => {
    return (
      <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <div style={{ fontSize: '0.72rem', fontWeight: 800, color: T.primary, textTransform: 'uppercase', letterSpacing: '1px' }}>Table Configuration</div>
        <div style={{ display: 'flex', gap: '10px' }}>
          <div style={{ flex: 1 }}>
            <label style={lbl}>Columns</label>
            <input style={inp} type="number" min={1} max={10} value={baseConfig.cols ?? 2}
              onChange={e => { const raw = e.target.value; if (raw === '') return onChangeConfig({ cols: '' }); const cols = Math.max(1, Math.min(10, Number(raw))); const rows = baseConfig.baseRowCount || baseConfig.numRows || 1; adjustMatrixDimensions(pathPrefix, cols, rows, baseConfig, onChangeConfig); }}
              onBlur={e => { const cols = Math.max(1, Number(e.target.value) || 1); const rows = baseConfig.baseRowCount || baseConfig.numRows || 1; adjustMatrixDimensions(pathPrefix, cols, rows, baseConfig, onChangeConfig); }} />
          </div>
          <div style={{ flex: 1 }}>
            <label style={lbl}>Base Rows</label>
            <input style={inp} type="number" min={1} max={50} value={baseConfig.baseRowCount ?? baseConfig.numRows ?? 1}
              onChange={e => { const raw = e.target.value; if (raw === '') return onChangeConfig({ baseRowCount: '', numRows: '' }); const count = Math.max(1, Math.min(50, Number(raw))); adjustMatrixDimensions(pathPrefix, baseConfig.cols || 2, count, baseConfig, onChangeConfig); }}
              onBlur={e => { const count = Math.max(1, Number(e.target.value) || 1); adjustMatrixDimensions(pathPrefix, baseConfig.cols || 2, count, baseConfig, onChangeConfig); }} />
          </div>
        </div>
          ...
          {[
            ...
            return (
              <label key={opt.key} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text }}>
                <input type="checkbox" checked={isChecked} onChange={e => onChangeConfig({ [opt.key]: e.target.checked })} /> {opt.label}
              </label>
            );
          })}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {(baseConfig.colHeaders || []).map((headerText, idx) => (
              <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '10px', fontWeight: 700, color: T.mutedText, width: '35px', flexShrink: 0 }}>Col {idx + 1}</span>
                <input style={{ ...inp, flex: 2 }} type="text" value={headerText} placeholder={`Column ${idx + 1}`} onChange={e => { const newHeaders = [...(baseConfig.colHeaders || [])]; newHeaders[idx] = e.target.value; onChangeConfig({ colHeaders: newHeaders }); }} />
                <input style={{ ...inp, flex: 1 }} type="text" value={baseConfig.colWidths?.[idx] || ''} placeholder="Width (e.g. 30%, 150px)" onChange={e => { const newWidths = [...(baseConfig.colWidths || [])]; newWidths[idx] = e.target.value; onChangeConfig({ colWidths: newWidths }); }} />
              </div>
            ))}
          </div>
        </div>
        <button onClick={() => { const targetCols = baseConfig.cols || 2; const targetRows = baseConfig.baseRowCount || baseConfig.numRows || 1; adjustMatrixDimensions(pathPrefix, targetCols, targetRows, baseConfig, onChangeConfig); setActiveCellRow(0); setActiveCellCol(0); setMatrixPath(pathPrefix); setShowTableModal(true); }}
          style={{ marginTop: '6px', display: 'flex', alignItems: 'center', gap: '6px', background: 'none', border: `1px solid ${T.border}`, color: T.text, padding: '7px 10px', borderRadius: '6px', fontSize: '0.8rem', cursor: 'pointer', width: '100%', justifyContent: 'center' }}>
  ...
  const renderRepeatingGroupConfigUI = (baseConfig, onChangeConfig) => {
    const updateTemplateField = (fi, key, value) => { const tpl = [...(baseConfig.template || [])]; tpl[fi] = { ...tpl[fi], [key]: value }; onChangeConfig({ template: tpl }); };
    return (
      <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ fontSize: '0.72rem', fontWeight: 800, color: T.primary, textTransform: 'uppercase', letterSpacing: '1px' }}>Repeating Group Config</div>
        <div>
          <label style={lbl}>Add Button Label</label>
          <input style={inp} type="text" value={baseConfig.addLabel || 'Add Entry'} onChange={e => onChangeConfig({ addLabel: e.target.value })} placeholder="e.g. Add Founder" />
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}>
          <input type="checkbox" checked={!!baseConfig.allowImage} onChange={e => onChangeConfig({ allowImage: e.target.checked })} /> Include image upload field
        </label>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <label style={{ ...lbl, margin: 0 }}>Template Fields</label>
            <button onClick={() => { const uid = generateId(); const tpl = [...(baseConfig.template || []), { id: uid, label: 'New Field', type: 'text' }]; onChangeConfig({ template: tpl }); }}
              style={{ background: T.primaryLight, border: `1px solid ${T.primaryBorder}`, color: T.primary, padding: '3px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.72rem', fontWeight: 700 }}>+ Add Field</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div key={field.id || fi} style={{ background: T.surface2, border: `1px solid ${T.border}`, borderRadius: '7px', padding: '10px' }}>
                <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                  ...
                  <select style={{ ...inp, width: '90px', flex: 'none', background: T.selectBg }} value={field.type || 'text'} onChange={e => updateTemplateField(fi, 'type', e.target.value)}>
                    <option value="text">Text</option><option value="textarea">Textarea</option><option value="image">Image</option><option value="file">File</option><option value="select">Select</option><option value="date">Date</option><option value="number">Number</option><option value="email">Email</option><option value="currency">Currency</option><option value="boolean">Yes/No</option>
                  </select>
                </div>
                {field.type === 'select' && (
                  <div style={{ marginBottom: '6px' }}>
                    <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                      <input style={{ ...inp, flex: 1, fontSize: '0.75rem' }} type="text" id={`newTplOpt_${baseConfig.id}_${fi}`} placeholder="Type an option…"
                        onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); const val = e.target.value.trim(); if (!val) return; updateTemplateField(fi, 'options', [...(field.options || []), val]); e.target.value = ''; } }} />
                      <button onClick={() => { const input = document.getElementById(`newTplOpt_${baseConfig.id}_${fi}`); const val = input.value.trim(); if (!val) return; updateTemplateField(fi, 'options', [...(field.options || []), val]); input.value = ''; }}
                        style={{ background: T.primaryLight, border: `1px solid ${T.primaryBorder}`, color: T.primary, padding: '5px 10px', borderRadius: '5px', cursor: 'pointer', fontWeight: 700, fontSize: '0.72rem', whiteSpace: 'nowrap' }}>+ Add</button>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                      {(field.options || []).map((opt, oi) => (
                        <div key={oi} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: T.surface2, border: `1px solid ${T.border}`, borderRadius: '4px', padding: '4px 8px' }}>
                          ...
                          <button onClick={() => updateTemplateField(fi, 'options', (field.options || []).filter((_, idx) => idx !== oi))} style={{ background: 'none', border: 'none', color: T.danger, cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9rem', lineHeight: 1 }}>×</button>
                        </div>
                      ))}
                    </div>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.78rem', color: T.text, cursor: 'pointer', marginTop: '8px' }}>
                      <input type="checkbox" checked={!!field.allowCustom} onChange={e => updateTemplateField(fi, 'allowCustom', e.target.checked)} /> Allow custom additions
                    </label>
                  </div>
                )}
                <input style={{ ...inp, fontSize: '0.75rem', marginBottom: '6px' }} type="text" value={field.placeholder || ''} onChange={e => updateTemplateField(fi, 'placeholder', e.target.value)} placeholder="Placeholder text (optional)" />
                <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
                  <button onClick={() => { const tpl = [...(baseConfig.template || [])]; if (fi > 0) { [tpl[fi], tpl[fi-1]] = [tpl[fi-1], tpl[fi]]; onChangeConfig({ template: tpl }); } }} style={{ background: 'none', border: 'none', color: T.mutedText, cursor: 'pointer', padding: '2px 4px' }}><ArrowUp size={12} /></button>
                  <button onClick={() => { const tpl = [...(baseConfig.template || [])]; if (fi < tpl.length - 1) { [tpl[fi], tpl[fi+1]] = [tpl[fi+1], tpl[fi]]; onChangeConfig({ template: tpl }); } }} style={{ background: 'none', border: 'none', color: T.mutedText, cursor: 'pointer', padding: '2px 4px' }}><ArrowDown size={12} /></button>
                  <button onClick={() => { const tpl = (baseConfig.template || []).filter((_, i) => i !== fi); onChangeConfig({ template: tpl }); }} style={{ background: 'none', border: 'none', color: T.danger, cursor: 'pointer', padding: '2px 4px' }}><Trash2 size={12} /></button>
                </div>
              </div>
            ))}
          </div>
  ...
  const renderChartConfigUI = (baseConfig, onChangeConfig) => {
    return (
      <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ fontSize: '0.72rem', fontWeight: 800, color: T.primary, textTransform: 'uppercase', letterSpacing: '1px' }}>Chart Configuration</div>
        <div><label style={lbl}>Chart Type</label><select style={{ ...inp, background: T.selectBg }} value={baseConfig.chartType || 'bar'} onChange={e => onChangeConfig({ chartType: e.target.value })}><option value="bar">Bar Chart</option><option value="line">Line Chart</option><option value="area">Area Chart</option><option value="pie">Pie Chart</option></select></div>
        <div><label style={lbl}>Chart Title</label><input style={inp} type="text" value={baseConfig.title || ''} onChange={e => onChangeConfig({ title: e.target.value })} placeholder="e.g. Revenue by Quarter" /></div>
        <div><label style={lbl}>X-Axis Label</label><input style={inp} type="text" value={baseConfig.xAxisLabel || ''} onChange={e => onChangeConfig({ xAxisLabel: e.target.value })} placeholder="e.g. Quarter or Category" /></div>
        <div><label style={lbl}>Base Row Count</label><input style={inp} type="number" min={1} max={50} value={baseConfig.baseRowCount ?? 1} onChange={e => onChangeConfig({ baseRowCount: Math.max(1, Math.min(50, Number(e.target.value) || 1)) })} /></div>
        {baseConfig.chartType === 'pie' && (<div><label style={lbl}>Pie Chart Series</label><select style={{ ...inp, background: T.selectBg }} value={baseConfig.pieSeriesIndex ?? 0} onChange={e => onChangeConfig({ pieSeriesIndex: Number(e.target.value) })}>{(baseConfig.series || []).map((name, i) => (<option key={i} value={i}>{name || `Series ${i + 1}`}</option>))}</select></div>)}
        <div>
          <label style={lbl}>Pre-set Row Labels (Categories)</label>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {(baseConfig.rowLabels || []).map((rName, idx) => (
              <div key={idx} style={{ display: 'flex', gap: '6px' }}>
                <input style={{ ...inp, flex: 1 }} type="text" value={rName} onChange={e => { const newLabels = [...(baseConfig.rowLabels || [])]; newLabels[idx] = e.target.value; onChangeConfig({ rowLabels: newLabels }); }} placeholder="Row Label (e.g. TAM)" />
                <button onClick={() => { const newLabels = (baseConfig.rowLabels || []).filter((_, i) => i !== idx); onChangeConfig({ rowLabels: newLabels }); }} style={{ background: 'none', border: 'none', color: T.danger, cursor: 'pointer', padding: '0 4px' }}><Trash2 size={14} /></button>
              </div>
            ))}
            <button onClick={() => { const newLabels = [...(baseConfig.rowLabels || []), `Category ${(baseConfig.rowLabels || []).length + 1}`]; onChangeConfig({ rowLabels: newLabels }); }} style={{ background: T.surface2, border: `1px dashed ${T.border}`, color: T.mutedText, padding: '6px', borderRadius: '6px', fontSize: '0.75rem', cursor: 'pointer', marginTop: '4px' }}>+ Add Row Label</button>
          </div>
        </div>
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {(baseConfig.series || []).map((sName, idx) => (
              <div key={idx} style={{ display: 'flex', gap: '6px' }}>
                <input type="color" value={(baseConfig.colors || [])[idx] || '#3b82f6'} onChange={e => { const newColors = [...(baseConfig.colors || [])]; newColors[idx] = e.target.value; onChangeConfig({ colors: newColors }); }} style={{ width: '32px', height: '32px', padding: 0, border: 'none', borderRadius: '4px', cursor: 'pointer', flexShrink: 0 }} />
                <input style={{ ...inp, flex: 1 }} type="text" value={sName} onChange={e => { const newSeries = [...(baseConfig.series || [])]; newSeries[idx] = e.target.value; onChangeConfig({ series: newSeries }); }} placeholder="Series Name (e.g. Actual)" />
                <button onClick={() => { const newSeries = baseConfig.series.filter((_, i) => i !== idx); const newColors = baseConfig.colors.filter((_, i) => i !== idx); onChangeConfig({ series: newSeries, colors: newColors }); }} style={{ background: 'none', border: 'none', color: T.danger, cursor: 'pointer', padding: '0 4px' }}><Trash2 size={14} /></button>
              </div>
            ))}
            <button onClick={() => { const newSeries = [...(baseConfig.series || []), `Series ${(baseConfig.series || []).length + 1}`]; const newColors = [...(baseConfig.colors || []), '#8b5cf6']; onChangeConfig({ series: newSeries, colors: newColors }); }} style={{ background: T.surface2, border: `1px dashed ${T.border}`, color: T.mutedText, padding: '6px', borderRadius: '6px', fontSize: '0.75rem', cursor: 'pointer', marginTop: '4px' }}>+ Add Series</button>
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '6px' }}>
              {(baseConfig.pieColors || ['3b82f6', '10b981', 'f59e0b', '8b5cf6', 'ec4899']).map((hex, idx) => (
                <div key={idx} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  <input type="color" value={`#${hex}`} onChange={e => { const newPieColors = [...(baseConfig.pieColors || ['3b82f6', '10b981', 'f59e0b', '8b5cf6', 'ec4899'])]; newPieColors[idx] = e.target.value.replace('#', ''); onChangeConfig({ pieColors: newPieColors }); }} style={{ width: '32px', height: '32px', padding: 0, border: 'none', borderRadius: '4px', cursor: 'pointer', flexShrink: 0 }} />
                  <span style={{ fontSize: '0.78rem', color: T.mutedText }}>Slice {idx + 1}</span>
                  <button onClick={() => { const newPieColors = (baseConfig.pieColors || ['3b82f6', '10b981', 'f59e0b', '8b5cf6', 'ec4899']).filter((_, i) => i !== idx); onChangeConfig({ pieColors: newPieColors }); }} style={{ background: 'none', border: 'none', color: T.danger, cursor: 'pointer', padding: '0 4px' }}><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
            <button onClick={() => { const newPieColors = [...(baseConfig.pieColors || ['3b82f6', '10b981', 'f59e0b', '8b5cf6', 'ec4899']), '94a3b8']; onChangeConfig({ pieColors: newPieColors }); }} style={{ background: T.surface2, border: `1px dashed ${T.border}`, color: T.mutedText, padding: '6px', borderRadius: '6px', fontSize: '0.75rem', cursor: 'pointer', width: '100%' }}>+ Add Slice Color</button>
          </div>
        )}  
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '8px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}><input type="checkbox" checked={baseConfig.showLegend !== false} onChange={e => onChangeConfig({ showLegend: e.target.checked })} /> Show Legend</label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}><input type="checkbox" checked={baseConfig.allowAddRows !== false} onChange={e => onChangeConfig({ allowAddRows: e.target.checked })} /> Allow adding rows</label>
        </div>
      </div>
    );
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {(blockConfig.type === 'boolean' || blockConfig.type === 'compliance') && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px' }}>
            ...
            <input style={inp} type="text" value={(blockConfig.options || []).join(', ')} onChange={e => onChangeConfig({ options: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })} placeholder="Yes, No, NA" />
          </div>
        )}
        {blockConfig.type === 'mixed' && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px' }}>
            <label style={{ ...lbl, color: T.primary }}>Template Blueprint</label>
            <textarea style={{ ...inp, resize: 'vertical', fontFamily: 'monospace', fontSize: '0.8rem' }} rows={4} value={blockConfig.template || ''} onChange={e => onChangeConfig({ template: e.target.value })} placeholder="e.g. Total [number] employees as of [date]" />
            <div style={{ fontSize: '0.68rem', color: T.mutedText, marginTop: '4px' }}>Tags: [text] [number] [date] [select] <br />Add placeholder: [text:Company name] [number:Revenue in ₹]</div>
          </div>
        )}
        {blockConfig.type === 'image' && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}><input type="checkbox" checked={!!blockConfig.multiple} onChange={e => onChangeConfig({ multiple: e.target.checked })} /> Allow multiple images</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}><input type="checkbox" checked={blockConfig.allowCaption !== false} onChange={e => onChangeConfig({ allowCaption: e.target.checked })} /> Show caption / name field</label>
            <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 10, display: 'flex', gap: 10 }}>
              <div style={{ flex: 1 }}><label style={lbl}>Display Width</label><input style={inp} type="text" value={blockConfig.imageWidth || '100%'} onChange={e => onChangeConfig({ imageWidth: e.target.value })} placeholder="e.g. 100% or 300px" /></div>
              <div style={{ flex: 1 }}><label style={lbl}>Display Height</label><input style={inp} type="text" value={blockConfig.imageHeight || '180px'} onChange={e => onChangeConfig({ imageHeight: e.target.value })} placeholder="e.g. 180px or auto" /></div>
            </div>
            <div><label style={lbl}>Image Fit</label><select style={{ ...inp, background: T.selectBg }} value={blockConfig.objectFit || 'cover'} onChange={e => onChangeConfig({ objectFit: e.target.value })}><option value="cover">Cover — crop to fill</option><option value="contain">Contain — letterbox</option><option value="fill">Fill — stretch to fit</option></select></div>
          </div>
        )}
        {blockConfig.type === 'file' && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px' }}>
            <label style={{ ...lbl, color: T.primary }}>File Options</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text, cursor: 'pointer', marginTop: '6px' }}><input type="checkbox" checked={!!blockConfig.multiple} onChange={e => onChangeConfig({ multiple: e.target.checked })} /> Allow multiple files</label>
            <div style={{ marginTop: '10px' }}><label style={lbl}>Accepted file types (e.g. .pdf,.xlsx)</label><input style={inp} type="text" value={blockConfig.accept || ''} onChange={e => onChangeConfig({ accept: e.target.value })} placeholder=".pdf,.xlsx,.docx" /></div>
          </div>
        )}
        {blockConfig.type === 'list' && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}><input type="checkbox" checked={!!blockConfig.autoSno} onChange={e => onChangeConfig({ autoSno: e.target.checked })} /> Auto-number rows</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: T.text, cursor: 'pointer' }}><input type="checkbox" checked={!!blockConfig.allowReorder} onChange={e => onChangeConfig({ allowReorder: e.target.checked })} /> Allow drag-to-reorder</label>
          </div>
        )}
        ...
        {blockConfig.type === 'conditional-switch' && (() => {
          const branches = blockConfig.branches || [ { id: 'branchA', label: blockConfig.branchA_label || 'Option A', blocks: blockConfig.branchA_blocks || [] }, { id: 'branchB', label: blockConfig.branchB_label || 'Option B', blocks: blockConfig.branchB_blocks || [] } ];
          ...
          return (
            <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: '16px' }}>
              ...
              <div style={{ display: 'flex', borderBottom: `1px solid ${T.border}`, marginTop: '14px', flexWrap: 'wrap', gap: '4px', paddingBottom: '8px' }}>
                {branches.map((b, i) => (
                  <button key={b.id} onClick={() => setInspectorTab(b.id)} style={{ padding: '6px 12px', border: 'none', borderRadius: '4px', background: activeTab === b.id ? T.primaryLight : 'transparent', color: activeTab === b.id ? T.primary : T.mutedText, fontWeight: activeTab === b.id ? 800 : 600, fontSize: '0.75rem', cursor: 'pointer', transition: 'all 0.15s ease' }}>{b.label || `Option ${i + 1}`}</button>
                ))}
                <button onClick={() => { const newId = `branch_${generateId()}`; const newBranches = [...branches, { id: newId, label: 'New Option', blocks: [] }]; onChangeConfig({ branches: newBranches }); setInspectorTab(newId); }} style={{ padding: '6px 10px', border: `1px dashed ${T.border}`, background: 'transparent', color: T.mutedText, borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}><Plus size={12}/> Add Option</button>
              </div>
              {activeBranch && (
                <div style={{ background: T.surface2, padding: '12px', borderRadius: '0 0 8px 8px', border: `1px solid ${T.border}`, borderTop: 'none', animation: 'imFadeIn 0.2s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    ...
                    {branches.length > 2 && (<button onClick={() => { const newBranches = branches.filter(b => b.id !== activeBranch.id); onChangeConfig({ branches: newBranches }); setInspectorTab(newBranches[0]?.id); }} style={{ background: 'none', border: 'none', color: T.danger, cursor: 'pointer', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '3px', fontWeight: 600 }}><Trash2 size={10} /> Delete Option</button>)}
                  </div>
                  <input style={{ ...inp, marginBottom: '16px' }} type="text" value={activeBranch.label || ''} onChange={e => { const newBranches = [...branches]; newBranches[activeBranchIdx] = { ...activeBranch, label: e.target.value }; onChangeConfig({ branches: newBranches }); }} placeholder="Button Label" />
                  <label style={lbl}>Sub-Blocks inside this option</label>
                  {renderSubBlockList(activeBranch.blocks || [], (arr) => { const newBranches = [...branches]; newBranches[activeBranchIdx] = { ...activeBranch, blocks: arr }; onChangeConfig({ branches: newBranches }); }, `${pathPrefix}.branches.${activeBranchIdx}.blocks`)}
                </div>
              )}
            </div>
          );
        ...
        {blocksArray.map((sb, idx) => (
          <div key={sb.id} style={{ padding: '12px', background: T.panelBg, border: `1px solid ${T.border}`, borderRadius: '8px', borderLeft: `3px solid ${T.primary}` }}>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              <select style={{ ...inp, flex: 1, fontWeight: 700 }} value={sb.type} onChange={e => { const next = [...blocksArray]; const tDef = BLOCK_TYPES.find(t => t.id === e.target.value); next[idx] = { ...sb, type: e.target.value, label: `New ${tDef.label}`, ...(BLOCK_DEFAULTS[e.target.value] || {}) }; onChangeArray(next); }}>
                {BLOCK_TYPES.filter(t => t.id !== 'repeating-block-set' && t.id !== 'conditional-switch').map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
              <button onClick={() => onChangeArray(blocksArray.filter((_, i) => i !== idx))} style={{ background: 'rgba(239,68,68,0.1)', border: 'none', color: T.danger, cursor: 'pointer', padding: '0 8px', borderRadius: '4px' }}><Trash2 size={15}/></button>
            </div>
            <input style={{ ...inp, marginBottom: '8px' }} value={sb.label || ''} onChange={e => { const next = [...blocksArray]; next[idx] = { ...next[idx], label: e.target.value }; onChangeArray(next); }} placeholder="Block Label" />
            <input style={{ ...inp, marginBottom: '12px', fontFamily: 'monospace', fontSize: '0.75rem', color: T.textMuted }} value={sb.dataPath || ''} onChange={e => { const next = [...blocksArray]; next[idx] = { ...next[idx], dataPath: e.target.value }; onChangeArray(next); }} placeholder="data_path_namespace" />
            {renderBlockSettings(sb, (updates) => { const next = [...blocksArray]; next[idx] = { ...next[idx], ...updates }; onChangeArray(next); }, `${pathPrefix}.${idx}`)}
          </div>
        ))}
        <button onClick={() => { const uid = generateId(); onChangeArray([...blocksArray, { id: uid, type: 'text', label: 'New Text Block', dataPath: `sub_${uid}` }]); }} style={{ background: T.primaryLight, color: T.primary, border: `1px dashed ${T.primaryBorder}`, padding: '10px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}><Plus size={14} /> Add Sub-Block</button>
      </div>
    );
  };
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: T.bg, color: T.text, fontFamily: '"Inter", sans-serif', overflow: 'hidden' }}>
      ...
      <header style={{ height: '60px', flexShrink: 0, background: T.headerBg, borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <button
            onClick={() => { if (projectId && fcId) navigate(`/fc?project=${projectId}&fc=${fcId}`); else navigate('/module-hub'); }}
          ...
          {/* FIX: Title updated for First Connect */}
          <h1 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 800, color: T.primary, letterSpacing: '1px' }}>FC SCHEMA BUILDER</h1>
          <button
